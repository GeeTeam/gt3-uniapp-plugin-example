 ...
 *  只要终端用户没有完成验证，返回就是0；只有完成验证，
 *  才会返回 code = 1 以及相应的二次校验 result 数据。
 *  code = 0 的具体场景，比如在滑动验证的时候，
 *  没有滑动正确。当 code = 0 时，验证码会自动刷新，
 *  终端用户充实即可。并且重试有最大的重试次数，
 *  根据验证类型不同，最大重试次数默认是 5-6 次。
 *
 *  @param manager 验证管理器
